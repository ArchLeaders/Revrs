...
public static class WriterExtensions
{
    ...
    /// <param name="endianness">The <see langword="byte-order"/> to use when writing the <paramref name="value"/>.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe void Write<T, TReverser>(this Stream stream, T value, Endianness endianness) where T : unmanaged where TReverser : IStructReverser
    {
        int size = sizeof(T);
        Span<byte> buffer = size <= 500_000
            ? stackalloc byte[size] : new byte[size];
    ...
    /// <param name="endianness">The <see langword="byte-order"/> to use when writing the <paramref name="value"/>.</param>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe void WriteStruct<T>(this Stream stream, T value, Endianness endianness) where T : unmanaged, IStructReverser
    {
        int size = sizeof(T);
        Span<byte> buffer = size <= 500_000
            ? stackalloc byte[size] : new byte[size];
